...
image_checker_agent = Agent(
    name="image_checker_agent",
    model_name="deepseek-v3-1-250821",
    instruction="""
        你是一个专业的AI图片评估师，擅长分析生成的图片是否符合给定的提示词要求。
        
        工作流程：
        1. 接收用户提供的提示词和图片URL
        2. 使用image_check工具调用doubao-seed-1-6-251015模型检查图片是否符合提示词要求
        3. 根据检查结果返回最终评估
        ...
        """,
    description="图片检查智能体，根据输入的提示词和图片链接分析图片是否符合提示词要求，如果不符合要求需要指出具体问题",
    tools=[image_check],
